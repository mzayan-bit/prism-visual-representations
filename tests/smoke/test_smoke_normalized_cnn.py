"""End-to-end smoke test for Normalized CNN with feature distribution tracking.

Guarantees:
- Full pipeline: Experiment -> Data -> Normalized CNN -> Reps -> Summaries -> Eval
- BatchNorm2D channel-wise normalization with running statistics tracking
- Extraction of pre-norm, post-norm, and final representations
...
@pytest.mark.smoke
def test_smoke_normalized_cnn_full_lifecycle() -> None:
    """Demonstrate end-to-end Normalized CNN baseline with distribution tracking."""
    ...
    # 3. Assemble Normalized Convolutional Neural Network Specification
    model_spec = ModelSpecification(
        model_id="model-cnn-bn-smoke",
        name="Normalized CNN Smoke Baseline",
    ...
    experiment = ExperimentDefinition(
        experiment_id="exp-cnn-bn-smoke-001",
        name="Normalized CNN Smoke Experiment",
    ...
    # 9. Verify Spatial Feature Map, Normalization and Vector Extraction
    model = ConvolutionalNeuralNetwork(spec=model_spec, seed=42)
    sample_batch = [test_dataset[0].data, test_dataset[1].data]

    # Pre-norm vs Post-norm extraction
    pre_norm_map = model.extract_representations(sample_batch, layer="conv_0_pre_norm")
    post_norm_map = model.extract_representations(
        sample_batch, layer="conv_0_post_norm"
    )

    assert len(pre_norm_map) == 2
    assert len(pre_norm_map[0]) == 8
    assert len(post_norm_map) == 2
    assert len(post_norm_map[0]) == 8

    # 10. Compute and Compare Feature Distribution Summaries
    summary_pre = compute_distribution_summary(pre_norm_map)
    summary_post = compute_distribution_summary(post_norm_map)

    assert summary_pre.is_finite is True
    assert summary_post.is_finite is True
    assert summary_pre.tensor_shape == (2, 8, 8, 8)
    assert summary_post.tensor_shape == (2, 8, 8, 8)
    assert len(summary_post.channel_means or []) == 8

    stability_delta = compare_distribution_summaries(summary_pre, summary_post)
    assert "mean_shift" in stability_delta
    assert "std_shift" in stability_delta

    # 11. Controlled Comparison Helper
    comp = create_normalization_comparison(
        comparison_id="comp-smoke-norm",
        name="Smoke Normalization Comparison",
        baseline_experiment_id="exp-cnn-smoke-001",
        candidate_experiment_id="exp-cnn-bn-smoke-001",
        dataset_fingerprint=canonical.compute_fingerprint(),
        seed=42,
    )
    assert comp.compute_fingerprint() is not None

    # 12. Build Representation Batch container
    desc = RepresentationDescriptor(
        layer_name="conv_0_post_norm",
        feature_dim=64,
        num_samples=2,
    ...
    rep_batch = RepresentationBatch(descriptor=desc, embeddings=post_norm_map)
